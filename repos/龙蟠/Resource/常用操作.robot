...
登录
    ...
    ####启动浏览器
    Open Browser    ${GLOBAL_CONFIG.url}    chrome    #打开浏览器
    界面最大化
    Input Text    id=staff_id    ${GLOBAL_CONFIG.username}    #输入用户名
    Input Text    id=password    ${GLOBAL_CONFIG.password}    #输入密码
...
选择图片
    ...
    ####选择图片
    ${图片总数}    Get Matching Xpath Count    xpath=//*[@id="select_ajax_form"]/div[3]/div/ul/li[@class="pic_list ng-scope"]
    ${图片}    Evaluate    random.randint(1,${图片总数})    random
...
微信模拟页面登录
    [Arguments]    ${客户ID}    ${name}=default
    ###模拟微信前端页面登录
    ${name}    Open Browser    ${GLOBAL_CONFIG.wx_url}/oauth/testing?id=${客户ID}    chrome
    Maximize Browser Window
    Set Selenium Timeout    20
    sleep    1
    [Return]    ${name}
...
计算邮费
    [Arguments]    ${shipModeTypeID}    ${productWeight}    ${productNum}    ${provinceName}    ${cityName}
    ${shipmodeInfor}    获取邮费模板信息    ${shipModeTypeID}
    #${provinceList}    Get From Dictionary    ${shipmodeInfor}    provinceList    #省列表
    ${shipmode}    Get From Dictionary    ${shipmodeInfor}    model
    ${shipmodeName}    Get From Dictionary    ${shipmode}    name    #邮费模板名称
    ${defaultFee}    Get From Dictionary    ${shipmode}    default    #首重邮费
    ${defaultWeight}    Get From Dictionary    ${shipmode}    w_default    #首重
    ${increaseFee}    Get From Dictionary    ${shipmode}    increase    #续重金额
    ${increaseWeight}    Get From Dictionary    ${shipmode}    w_increase    #续重
    ${productsShippingFees}    Get From Dictionary    ${shipmode}    productsShippingFees
    ${specialShipMode}    Get From List    ${productsShippingFees}    0
    #${specialArea}    Get From Dictionary    ${specialShipMode}    area    #特殊地区ID
    ${specialAreaText}    Get From Dictionary    ${specialShipMode}    area_text    #特殊地区
    ${specialDefaultFee}    Get From Dictionary    ${specialShipMode}    default    #特殊地区首重金额
    ${specialIncreaseFee}    Get From Dictionary    ${specialShipMode}    increase    #特殊地区续重金额
    ${newSpecialAreaText}    Remove String    ${specialAreaText}    (    )    ;
    ${specialAreaTextList}    Split String    ${newSpecialAreaText}    ,
    ${isProvinceContain}    Get Index From List    ${specialAreaTextList}    ${provinceName}
    ${isCityContain}    Get Index From List    ${specialAreaTextList}    ${cityName}
    ${totalWeight}    Evaluate    ${productWeight} * ${productNum}
    ${subTotalWeight}    Run Keyword If    ${defaultWeight} > ${totalWeight}    Set Variable    0
    ...    ELSE    Evaluate    ${totalWeight} - ${defaultWeight}
    ${subTotalWeightNumber}    Convert To Number    ${subTotalWeight}
    ${timesWeightNumber}    Run Keyword If    ${subTotalWeight} > 0    Evaluate    ${subTotalWeightNumber} / ${increaseWeight}
    ${timesWeight}    Run Keyword If    ${subTotalWeight} > 0    Evaluate    math.ceil(${timesWeightNumber})    modules=math
    ${extraFee}    Run Keyword If    ${subTotalWeight} > 0    Evaluate    ${timesWeight} * ${increaseFee}
    ...    ELSE    Set Variable    0
    ${specialExtraFee}    Run Keyword If    ${subTotalWeight} > 0    Evaluate    ${timesWeight} * ${specialIncreaseFee}
    ...    ELSE    Set Variable    0
    ${totalFee}    Evaluate    ${extraFee} + ${defaultFee}
    ${specialTotalFee}    Evaluate    ${specialDefaultFee} + ${specialExtraFee}
    ${returnValue}    Set Variable If    ${isCityContain} \ >= 0    ${specialTotalFee}    ${totalFee}
    [Return]    ${returnValue}
